...
describe('OI4MessageBus test', () => {
    ...
    it('should replace old config with new config and emit status status via mqtt process', async () => {
        ...
        expect(eventEmitMock).toHaveBeenCalledWith(MqttMessageProcessorEventStatus.SET_CONFIG, new StatusEvent(defaultOi4ApplicationResources.oi4Id.toString(), EOPCUAStatusCode.Good));

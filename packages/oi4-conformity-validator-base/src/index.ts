...
import dataSetClassIds = require('oi4-service-node/src/Config/Constants/dataSetClassIds.json'); /*tslint:disable-line*/
import {EventEmitter} from 'events';
import {EValidity, IConformity, ISchemaConformity, IValidityDetails} from './models/IConformityValidator';
...
import {
  ...
  uint16SchemaJson,
  uint32SchemaJson
} from 'oi4-service-node/src/Utilities/SchemaProvider';

// DSCIds
// DSWIds
import {IDataSetClassIds} from 'oi4-service-node/src/Models/IContainer';

// Resource imports
import resourceLookup from 'oi4-service-node/src/Config/Constants/resources.json'; /*tslint:disable-line*/
import Ajv from 'ajv'; /*tslint:disable-line*/
import {Logger} from 'oi4-service-node/src/Utilities/Logger';
import {IOPCUANetworkMessage} from 'oi4-service-node/src/Models/IOPCUA';
import {EAssetType, ESyslogEventFilter} from 'oi4-service-node/src/Enums/EContainer';
...
export class ConformityValidator extends EventEmitter {
  ...
  async checkResourceConformity(fullTopic: string, tag: string, resource: string): Promise<IValidityDetails> {
    ...
    const conformityPayload = this.builder.buildOPCUANetworkMessage([], new Date, dscids[resource]);
